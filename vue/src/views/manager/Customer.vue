<template>
  <div>
    ...
    <div class="operation">
      <el-button type="primary" plain @click="handleAdd">新增用户信息</el-button>
    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="zipCode" label="用户邮编"></el-table-column>
        ...
        <el-table-column prop="status" label="用户状态">
          <template v-slot="scope">
            <el-tag type="success" v-if="scope.row.status === '成交用户'">成交用户</el-tag>
            <el-tag type="info" v-if="scope.row.status === '潜在用户'">潜在用户</el-tag>
            <el-tag type="primary" v-if="scope.row.status === '长期用户'">长期用户</el-tag>
            <el-tag type="danger" v-if="scope.row.status === '资深用户'">资深用户</el-tag>
          </template>
        </el-table-column>

        <el-table-column prop="level" label="用户级别" width="120">
          <template v-slot="scope">
            <el-tag type="info" v-if="scope.row.level === '无级别'">无级别</el-tag>
            <el-tag type="success" v-if="scope.row.level === '普通用户'">普通用户</el-tag>
            <el-tag type="primary" v-if="scope.row.level === '高级用户'">高级用户</el-tag>
            <el-tag type="danger" v-if="scope.row.level === 'VIP用户'">VIP用户</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog title="用户表" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="用户邮编" prop="zipCode">
          <el-input v-model="form.zipCode" placeholder="用户邮编"></el-input>
        </el-form-item>
        ...
        <el-form-item label="用户状态" prop="status">
          <el-select style="width: 100%" v-model="form.status">
            <el-option v-for="item in ['潜在用户', '成交用户','长期用户','资深用户']" :key="item" :value="item" :label="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="用户级别" prop="level">
          <el-select style="width: 100%" v-model="form.level">
            <el-option v-for="item in ['无级别','普通用户', '高级用户','VIP用户']" :key="item" :value="item" :label="item"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
